...
class DataIngestion:
    ...
    def download_housing_data(self) -> str:
        try:
            ...
            logging.info(f"filr: [{tgz_file_path}] has been downloaded successfully")
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            if strat_train_set is not None:
                os.makedirs(self.data_ingestion_config.ingested_train_dir,exist_ok=
                            True)
                logging.info(f"exporting training dataset to file: [{train_file_path}]")
                strat_train_set.to_csv(train_file_path,index=False)

            if strat_test_set is not None:
                os.makedirs(self.data_ingestion_config.ingested_test_dir, exist_ok=
                True)
    ...
    def initiate_data_ingesion(self) ->DataIngestionArtifact:
        try:
    ...
    def __del__(self):
        logging.info(f"{'='*20}Data ingesion log completed {'='*20}\n\n")

...
class ModelTrainer:
    def __init__(self,model_trainer_config:ModelTrainerConfig,data_transformation_artifact: DataTransformationArtifact):
        try:
            logging.info(f"{'=' * 20}Model Training has started {'=' * 20}")
    ...
    def initiate_model_trainer(self):
        try:
            ...
            logging.info("splitting training and testing data into input and target features")
            x_train,y_train,x_test,y_test =train_array[:,:-1],train_array[:,-1],test_array[:,:-1],test_array[:,-1]

            logging.info("Extracting model config file path")
            model_config_file_path = self.model_trainer_config.model_config_file_path

            logging.info(f"initializing model factory class using model config file path: {model_config_file_path}")
            model_factory = ModelFactory(model_config_path=model_config_file_path
                                         )
            base_accuracy = self.model_trainer_config.base_accuracy
            logging.info(f"expected accuracy {base_accuracy}")

            logging.info(f"initializing model selection")
            best_model = model_factory.get_best_model(X=x_train,y=y_train,
